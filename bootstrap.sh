...
cat > ./bind/config/named.conf <<- EOF 
acl internal {
  ${IP_RANGE}; # Only allow queries from within ipvlan
};

options {
  forwarders {
    ${UNBOUND_IP}; # Forward to unbound if not auth
  };
...
serial=$(printf "\t%24.s$(date +%Y%m%d)00")

cat > ./bind/config/${DOMAIN}.zone <<- EOF
; how long records are valid before new query
\$TTL 86400
...
@               IN      SOA     ns.${DOMAIN}. root.localhost. (
				${serial}      ; serial number (REMEMBER TO INCREMENT AFTER UPDATING!)
                                12h             ; refresh
                                15m             ; update retry
                                3w              ; expiry
...
; add dns records below here
router          IN      A       ${ROUTER_IP}
unbound         IN      A       ${UNBOUND_IP}
pihole          IN      A       ${PIHOLE_IP}
EOF
...
server:
    ...
    access-control: ${BIND9_IP} allow # Only give access to bind9 server
...
SCRIPT_PATH="/usr/local/bin/${VIRTUAL_IFACE}.sh"

cat > ${SCRIPT_PATH} <<- EOF
#!/bin/sh
ip link add ${VIRTUAL_IFACE} link ${PARENT_IFACE} type ipvlan mode l2
ip addr add ${RESERVED_IP} dev ${VIRTUAL_IFACE}
ip link set ${VIRTUAL_IFACE} up
ip route add ${IP_RANGE} dev ${VIRTUAL_IFACE}
EOF

chmod +x ${SCRIPT_PATH} && \. ${SCRIPT_PATH}

cat > /etc/systemd/system/${VIRTUAL_IFACE}.service <<- EOF
[Unit]
...
systemctl enable ${VIRTUAL_IFACE}.service
